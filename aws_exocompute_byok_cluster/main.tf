...
module "cluster" {
  ...
  aws_eks_worker_node_role_arn            = module.account.aws_eks_worker_node_role_arn
  ...
  aws_iam_cross_account_role_arn          = module.account.aws_iam_cross_account_role_arn
...
# Take care with this resource as there is no way to disable the private
# registry once it has been enabled.
...
# Deploy K8s resource from the manifest to the EKS cluster.
module "manifest" {
  source = "./manifest"
