...
data "aws_ssm_parameter" "worker_image" {
  name = "/aws/service/eks/optimized-ami/${var.aws_eks_kubernetes_version}/amazon-linux-2/recommended/image_id"
}

resource "aws_launch_template" "worker" {
  name                   = "${var.aws_name_prefix}-rubrik-exocompute-worker-${data.aws_region.current.name}"
